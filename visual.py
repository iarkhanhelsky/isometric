...
def draw_object(obj):
    ...
    basement = [vecadd(ij_to_xy(*p), (0, -level)) for p in [(0, 0), (0, 0 + cols), (0 + rows, 0 + cols), (0 + rows, 0)]]
    ...
    v_move = -basement[3][1]
    delta = ij_to_xy(i, j)
    rect = (delta[0], delta[1] - (height + level), basement[2][0], basement[1][1] - basement[3][1] + height)
    ...
    return (rect[0], rect[1] - v_move), surf